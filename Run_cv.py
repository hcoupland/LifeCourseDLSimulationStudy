...
def hyperparameter_optimise(model_name,Xtrainvalid,Ytrainvalid, epochs, randnum, num_optuna_trials):
    ...
    if model_name=="LSTMFCN":
        trial=MLmodel_opt.hypersearchLSTMFCN(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="TCN":
        trial=MLmodel_opt.hypersearchTCN(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="XCM":
        trial=MLmodel_opt.hypersearchXCM(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="ResCNN":
        trial=MLmodel_opt.hypersearchResCNN(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="ResNet":
        trial=MLmodel_opt.hypersearchResNet(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="InceptionTime":
        trial=MLmodel_opt.hypersearchInceptionTime(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)

    if model_name=="MLSTMFCN":
         trial=MLmodel_opt.hypersearchMLSTMFCN(Xtrainvalid,Ytrainvalid,epochs=epochs,randnum=randnum, num_optuna_trials=num_optuna_trials)
...
def All_run(name,model_name,Xtrainvalid, Ytrainvalid, Xtest, Ytest, splits, X, Y, randnum=8,  epochs=10,num_optuna_trials = 100, hype=False):
    ...
    if model_name=="LR":
        ...
        if hype=="True":
            ...
            runtime, acc, prec, rec, fone, auc, prc, LR00, LR01, LR10, LR11 = MLmodel_opt.model_block(arch=arch,X=X,Y=Y,splits=splits,randnum=randnum,epochs=epochs,params=params,lr_max=lr_max,alpha=alpha,gamma=gamma,batch_size=batch_size)
            ...
            runtime, acc, prec, rec, fone, auc, prc, LR00, LR01, LR10, LR11 = MLmodel_opt.model_block_nohype(arch=arch,X=X,Y=Y,splits=splits,randnum=randnum,epochs=epochs,lr_max=lr_max,alpha=alpha,gamma=gamma,batch_size=batch_size)
